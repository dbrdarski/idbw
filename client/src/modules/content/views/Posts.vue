    ...
    <PageLayout>
        <div class="q-pa-lg" style="max-width: 1200px; margin: auto">
            ...
            <div class="row q-py-md">
                <q-btn-group outline>
                    <template v-for="(filterItem, filterKey) in filters" :key="filterKey">
                        <q-separator v-if="filterKey !== Object.keys(filters)[0]" />
                        <q-btn :label="filterItem.label" :outline="filterKey !== filter" color="blue-7"
                            @click="filter = filterKey" />
                    </template>
                </q-btn-group>
                <q-space />
                <q-btn label="Create new" noshadow color="primary" unelevated size="md" @click="openPost()" />
            </div>
            <q-table :columns="columns" :rows="entries" v-model:pagination="pagination" no-shadow align="left"
                @request="onReq">
                <template v-slot:body-cell-actions="props">
                    <q-td align="right">
                        <q-btn icon="inventory" color="grey-8" size="sm" round flat @click="archivePost(props.row)" />
                        <q-btn icon="edit" color="grey-8" size="sm" round flat @click="openPost(props.row)" />
                    </q-td>
                </template>
            </q-table>
        </div>
